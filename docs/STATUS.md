...
| 5-model access PDP | Div IV | ✅ | RBAC·ReBAC·ABAC·PBAC·CABAC, deny-wins, tested | Enforcement wired into every route/action |
...
3. **Enforce the guardrails everywhere** — apply the access PDP and consent gates to every route/server action (currently the engines exist and are tested, but enforcement is not yet threaded through all pages).
