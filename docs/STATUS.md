...
| Per-role data scoping (ReBAC) | ✅ (engine) / 🟡 (rollout) | `lib/access/scope` + `scope-server` + `/governance/scope`; **wired on 15 modules**: safety, discipline, cwsn, lost-found, cooks, rte, rti, oosc, water, cctv, drills, competitions, excursions, tc, visitors. Remaining store-backed modules pending |
