...
| All interactive modules persisted | 🟡 | ~43 modules persist via stores (+ Postings, Notices, Sports, MDM, Scholarships; plus Leave/SMC/Recognition/Admissions/Grievance workflow flows); **~31 modules still hold state in the browser (`useState`)**. They follow the same converter — rolling out module-by-module |
