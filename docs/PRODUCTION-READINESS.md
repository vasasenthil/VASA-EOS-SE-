...
| All interactive modules persisted | 🟡 | ~50 modules persist via stores (+ Visitors, Circulation, Alumni, Diagnostic/Results snapshots, Maintenance-flow, staff-attendance snapshots); plus the Leave/SMC/Recognition/Admissions/Grievance/Maintenance workflow flows; **~24 modules still hold state in the browser (`useState`)** — mostly computed/seeded views (timetable, SIS, exam-seating planner). Rolling out module-by-module |
