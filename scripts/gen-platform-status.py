...
DESC = {
    ...
    "attendance": "L6 Student Attendance: high-volume daily marks (present/absent/late/excused) · upsert (re-mark corrects) · per-student attendance rate · RTE CHRONIC-ABSENTEE early-warning (<75% over >=10 days, excused-exempt) · jurisdiction-scoped dashboard · DURABLE PostgreSQL store",
    ...
    "scholarship": "L6 Scholarship/DBT: amount-driven multi-level fund sanction (school·BEO · +DEO >Rs50k · +directorate >Rs2L, PFMS/GFR) → disburse with payment ref → reconcile vs the rail (unmatched=leakage flag) · money in paise · scoped DBT dashboard · DURABLE PostgreSQL store",
    "leave": "L6 Staff Leave & Approval: file → DYNAMIC multi-level approval (principal → +BEO >5d → +DEO >15d) · scoped approval inbox · DURABLE PostgreSQL store · the Next.js leave-approval flow calls platformd over HTTP (lib/platform-client) — the frontend↔backbone connection",
    "fees": "L6 Fee & Finance Ledger: raise fee demands against students (exam·hostel·special heads) → collect payments → waive (concession) · MONEY IN PAISE (int64, never floats) · NO-OVERPAYMENT invariant — a payment can never take the collected total above the amount demanded (enforced against the durable total, atomically with the pending→partial→paid status recompute) · a paid/waived demand is closed to further payment · collection % + defaulter roster (open demands past due) · jurisdiction-scoped finance dashboard · DURABLE PostgreSQL store",
    "establishment": "L6 Staff Establishment & Sanctioned-Post Register: sanctioned posts per school cadre (Headmaster·BT·SGT·PET·Office Assistant) → appoint staff against them → vacate (frees a post) · OVER-APPOINTMENT GATE — the filled posts of a cadre can never exceed its sanctioned strength (enforced against the durable filled count) · an employee holds at most one filled post per establishment · a frozen establishment takes no appointments · sanctioned-vs-filled strength + vacancy % + vacancy roster · jurisdiction-scoped staffing dashboard · DURABLE PostgreSQL store",
    "entitlement": "L6 Free-Supply Entitlement Distribution: grant each student their entitlement under TN's free-supply schemes (textbooks·notebooks·uniforms·shoes·bags·cycles) → issue distributions against it · OVER-ISSUE GATE — a student can never be issued more than their entitlement (the leakage gate, enforced against the durable issued total atomically with the pending→partial→fulfilled status recompute) · a fulfilled/cancelled entitlement takes no more issues · per-item fulfilment % + shortfall worklist · jurisdiction-scoped distribution dashboard · DURABLE PostgreSQL store",
    "ptm": "L6 Parent-Teacher Meeting: schedule PTM sessions (fixed slots) → guardians book slots → mark attendance (booked → attended | no_show, or cancel) · OVERBOOKING invariant — a session can never exceed its slots · a guardian can never double-book the same session · a cancelled session takes no bookings, a cancellation frees its slot · fill % + TURNOUT % (attended vs occupied) + low-turnout roster · jurisdiction-scoped parent-engagement dashboard · DURABLE PostgreSQL store",
    "immunisation": "L6 School Health Immunisation: record vaccine doses under the school-health schedule (UIP/RBSK: Td10·Td16·MR·JE·VitaminA·Albendazole) · SEQUENCE invariant — dose N requires doses 1..N-1 already given (no dose 2 before dose 1) · off-schedule + over-count + future-dated doses rejected · immunisation status (complete·partial·due) derived vs the schedule · sensitive health data — aggregate coverage public, per-child DUE WORKLIST officer-only · jurisdiction-scoped coverage dashboard · DURABLE PostgreSQL store",
    "infra": "L6 Infrastructure & Asset Register: school assets/rooms with a CONDITION grade (good·fair·poor·unusable) + a constraint-checked maintenance-ticket lifecycle (open → in_progress → resolved → closed, no skips) · a critical ticket AUTO-flips its asset to under_maintenance · an asset can NEVER be decommissioned (or returned to service) while it has open tickets · estate dashboard (condition/status rollup · open backlog by severity · needs-attention roster) · jurisdiction-scoped · DURABLE PostgreSQL store",
    "mdm": "L6 Mid-Day Meal (PM-POSHAN): per-school foodgrain stock ledger (receipts in · consumptions out) + daily meal-service register · CORE LEAKAGE GATE — stock can never go NEGATIVE (a day can never cook more grain than is on hand, enforced atomically with the draw-down in one transaction) · meals-served <= enrolment data-quality gate · grain in GRAMS (int64, never floats) · coverage % + days-of-cover LOW-STOCK roster · jurisdiction-scoped PM-POSHAN dashboard · DURABLE PostgreSQL store",
    "transport": "L6 School Transport route-safety: register bus routes (vehicle + driver with statutory validity dates) · seat students with TWO hard safety invariants — a route can never exceed its seating CAPACITY, and no child may be allotted to an UNSERVICEABLE vehicle (lapsed fitness certificate or driver licence) · withdraw frees a seat · utilisation analytics + the unserviceable-route SAFETY ROSTER · jurisdiction-scoped transport dashboard · DURABLE PostgreSQL store",
    "library": "L6 School Library: issue physical book copies to members (students/teachers) → renew (capped) → return | lost · ONE-COPY-ONE-BORROWER invariant (a single copy can never be on loan twice, enforced in SQL AND by a partial unique index) · overdue analytics as-of today · jurisdiction-scoped circulation dashboard (active · overdue · lost roster) · DURABLE PostgreSQL store",
    "timetable": "L6 School Timetable: assign class-slots (subject+teacher) · TEACHER-CLASH detection (a teacher can never be in two classes at the same day+period, enforced in SQL) · per-teacher weekly load + overload signal (>30 periods/wk) · class & teacher grid views · jurisdiction-scoped timetabling dashboard · DURABLE PostgreSQL store",
    "cpd": "L6 Teacher CPD: durable in-service training records (NISHTHA·SCERT·DIET·DIKSHA) · per-teacher annual hours · NEP-2020 compliance (>=50h/yr) · compliant-vs-deficient roster · jurisdiction-scoped compliance dashboard · DURABLE PostgreSQL store",
    ...
    "audit": "Immutable hash-chain + Merkle root (tamper-evident) · DURABLE PostgreSQL sink when DATABASE_URL set — the chain survives restarts, is re-verified on load, and refuses to start on a tampered history",
    "kms": "Envelope encryption · per-tenant KEK hierarchy · rotation",
    "pep": "Policy Enforcement Point over the Rego plane (fail-closed)",
    "directory": "User Directory & unified IAM: every user category bound to an org unit · one PDP composing all 5 models (RBAC·ABAC·ReBAC·PBAC·CABAC) · explainable per-model decision trace (Access Explorer) · DURABLE PostgreSQL user store + CRUD when DATABASE_URL set — decisions run over persisted users",
