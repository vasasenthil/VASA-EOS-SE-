...
export const BROCHURE_CLAIMS: BrochureClaim[] = [
  ...
  { id: "policy-as-code", area: "Governance & oversight", claim: "Policy-as-Code — Education Acts automate compliance in real time", status: "partial", note: "executable enforcement engine (RTE/RPwD/DPDP/POCSO/GFR as cited rules → permit/deny/require-approval, deny-wins) + live simulator at /governance/policy-engine. The runtime gate (policyGate) is now WIRED into 9 real high-stakes flows across 5 statutes — GeM procurement payment + DBT scholarship disbursement + school-grant allocation (GFR/PFMS fund-flow, with high-value releases routed to secretariat approval), RTE admissions (RPwD reasonable-accommodation), staff appointment (POCSO background-verification), transfer-certificate + bonafide-certificate issue (DPDP consent), fee-demand (RTE §13(1) no capitation/screening fee), exam-result processing (RPwD inclusive assessment) — each deny-wins and audits every decision to the integrity ledger, tracked by a self-verifying register (lib/governance/policy-enforced.ts + tests/policy-enforced.test.ts). Rules are real and tested; extending the gate to the remaining mutating flows across the catalogue is in progress", repoRef: "lib/governance/policy-enforced.ts" },
