...
export const BROCHURE_CLAIMS: BrochureClaim[] = [
  ...
  { id: "modules", area: "Governance & oversight", claim: "391 functional modules (catalogue: 312) across the tiers", status: "partial", note: "high breadth (~271 routes); 50 are now DEEP-transactional — clickable routes whose server actions drive the durable Go backbone (platformd + PostgreSQL) with real workflows/invariants/audited writes, self-verified by lib/governance/durable-modules.ts (tests/durable-modules.test.ts) plus committed Go invariant tests; the remaining catalogue is reference UI, so not all are deeply built", repoRef: "lib/governance/durable-modules.ts" },
  ...
  // --- Scale & assurance ---
  { id: "persistence", area: "Scale & assurance", claim: "Durable persistence at state scale", status: "partial", note: "50 self-migrating PostgreSQL stores behind the Go backbone (each adapter runs CREATE TABLE IF NOT EXISTS on boot) proven durable against PostgreSQL 16 — writes survive restarts; plus the Supabase service-role seam + migrations for the reference app. Falls back to in-memory until a DB is provisioned; a live load test at state scale is still pending", repoRef: "lib/persistence/schema.ts" },
