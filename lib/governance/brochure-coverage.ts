...
export const BROCHURE_CLAIMS: BrochureClaim[] = [
  ...
  { id: "modules", area: "Governance & oversight", claim: "391 functional modules (catalogue: 312) across the tiers", status: "partial", note: "high breadth (~271 routes); 35 are now DEEP-transactional — clickable routes whose server actions drive the durable Go backbone (platformd + PostgreSQL) with real workflows/invariants/audited writes, self-verified by lib/governance/durable-modules.ts (tests/durable-modules.test.ts); the remaining catalogue is reference UI, so not all are deeply built", repoRef: "lib/governance/durable-modules.ts" },
