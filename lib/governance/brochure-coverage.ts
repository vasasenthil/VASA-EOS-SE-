...
export const BROCHURE_CLAIMS: BrochureClaim[] = [
  ...
  // --- Native-AI ---
  { id: "agents", area: "Native-AI", claim: "Eight purpose-built AI agents under human authority (HITL)", status: "built", note: "eight agents (Policy/Teacher/Student/Governance/Grievance/Compliance/Parent & Community/Inclusion) with the full five-part anatomy, composing the engines into advisory recommendations; high-stakes/low-confidence require approval; tested, at /ai-fabric", repoRef: "lib/ai/agents/index.ts" },
  ...
  { id: "engines", area: "Native-AI", claim: "Eight AI engines (Reasoning, Personalisation, Assessment, Policy, Analytics, Conversational, Language & Speech, Prediction & Early-Warning)", status: "built", note: "eight deterministic, explainable, tested engines/engine plans (advisory — no side effects) at /ai-engines; language/speech and early-warning carry child-safety and named-human routing guardrails", repoRef: "lib/ai/engines/index.ts" },
  ...
  { id: "policy-as-code", area: "Governance & oversight", claim: "Policy-as-Code — Education Acts automate compliance in real time", status: "partial", note: "executable enforcement engine (RTE/RPwD/DPDP/POCSO/GFR as cited rules → permit/deny/require-approval, deny-wins) + live simulator at /governance/policy-engine. The runtime gate (policyGate) is now WIRED into 10 real high-stakes flows across 5 statutes — GeM procurement payment + DBT scholarship disbursement + school-grant allocation + financial Government-Order issue (GFR/PFMS fund-flow, with high-value releases routed to secretariat approval), RTE admissions (RPwD reasonable-accommodation), staff appointment (POCSO background-verification), transfer-certificate + bonafide-certificate issue (DPDP consent), fee-demand (RTE §13(1) no capitation/screening fee), exam-result processing (RPwD inclusive assessment) — each deny-wins and audits every decision to the integrity ledger, tracked by a self-verifying register (lib/governance/policy-enforced.ts + tests/policy-enforced.test.ts). Rules are real and tested; extending the gate to the remaining mutating flows across the catalogue is in progress", repoRef: "lib/governance/policy-enforced.ts" },
  ...
  { id: "modules", area: "Governance & oversight", claim: "392 functional modules (337 core + 55 TN-specific) across the tiers", status: "partial", note: "high breadth (~271 routes); 52 are now DEEP-transactional — clickable routes whose server actions drive the durable Go backbone (platformd + PostgreSQL) with real workflows/invariants/audited writes, self-verified by lib/governance/durable-modules.ts (tests/durable-modules.test.ts) plus committed Go invariant tests; the remaining catalogue is reference UI, so not all are deeply built", repoRef: "lib/governance/durable-modules.ts" },
