...
export const ARCHITECTURE_LAYERS: ArchitectureLayer[] = [
  ...
  },
  {
    ...
    status: "partial",
    note:
      "13 typed adapter ports (incl. dedicated PFMS fund-flow) sit behind one interface, with the federation console, NDEAR-S 29/29 alignment and drift detectors that compare upstream records against local masters — APAAR↔student (field-level) and state-EMIS↔enrolment (tolerance-aware count reconciliation), each advisory (Reconciled/Review/Flagged, human decides). Adapters are mock by default and live only when env-configured; the platform is not live-federated with the national registries at runtime.",
    components: ["Typed integration gateway (DIKSHA/UDISE+/APAAR/PFMS/DBT)", "NDEAR-S 29/29 alignment", "Drift detectors (APAAR↔student, EMIS↔enrolment)", "Federation reconciliation console (HITL)"],
