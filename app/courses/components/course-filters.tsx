...
import { COURSE_STATUSES, CLASS_LEVELS } from "@/lib/courses"
...
export function CourseFilters() {
  ...
  const hasFilters = !!sp.get("q") || status !== "all" || cls !== "all"
    ...
    <div className="mb-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5 items-end">
      <div className="lg:col-span-2">
        <form onSubmit={(e) => { e.preventDefault(); push({ q }) }} className="flex gap-2">
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search name, code or teacher…" />
      ...
      <Select value={cls} onValueChange={(v) => push({ class: v })}>
        ...
        <SelectContent>
          ...
          {CLASS_LEVELS.map((c) => <SelectItem key={c} value={c}>Class {c}</SelectItem>)}
        </SelectContent>
      </Select>
      <div className="flex gap-2">
        ...
        {hasFilters ? (
          <Button variant="ghost" size="icon" onClick={() => { setQ(""); router.push(pathname) }} aria-label="Clear filters">
