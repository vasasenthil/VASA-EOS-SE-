...
import { SUBJECT_AREAS, CLASS_LEVELS, PATHWAY_STATUSES } from "@/lib/pathways"

export function PathwayFilters() {
    ...
    <div className="mb-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5 items-end">
      ...
      <div className="flex gap-2">
        <Select value={planStatus} onValueChange={(v) => push({ planStatus: v })}>
          ...
          <SelectContent><SelectItem value="all">All</SelectItem>{PATHWAY_STATUSES.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
        </Select>
