...
import { GRADE_STATUSES, TERMS, CLASS_LEVELS, SUBJECT_AREAS } from "@/lib/grades"
...
export function GradeFilters() {
  ...
  const hasFilters = !!sp.get("q") || status !== "all" || cls !== "all" || subject !== "all" || term !== "all"
    ...
    <div className="mb-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-6 items-end">
      ...
      <div className="flex gap-2">
        <Select value={status} onValueChange={(v) => push({ status: v })}>
          ...
          <SelectContent><SelectItem value="all">All</SelectItem>{GRADE_STATUSES.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
        </Select>
