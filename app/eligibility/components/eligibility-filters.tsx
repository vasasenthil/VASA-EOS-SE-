...
import { CATEGORIES, DECISIONS } from "@/lib/eligibility"
...
export function EligibilityFilters() {
  ...
  const hasFilters = !!sp.get("q") || category !== "all" || decision !== "all"
    ...
    <div className="mb-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4 items-end">
      <div className="lg:col-span-2">
        <form onSubmit={(e) => { e.preventDefault(); push({ q }) }} className="flex gap-2">
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search subject or reference…" />
      ...
      <Select value={category} onValueChange={(v) => push({ category: v })}>
        ...
        <SelectContent><SelectItem value="all">All rule sets</SelectItem>{CATEGORIES.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
      </Select>
      <div className="flex gap-2">
        <Select value={decision} onValueChange={(v) => push({ decision: v })}>
          <SelectTrigger><SelectValue placeholder="Decision" /></SelectTrigger>
          <SelectContent><SelectItem value="all">All</SelectItem>{DECISIONS.map((d) => <SelectItem key={d} value={d}>{d}</SelectItem>)}</SelectContent>
        </Select>
